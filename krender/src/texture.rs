...
/// A texture is either loaded from disk or generated by the program. 
/// In either case, the dimension of the texture is decided by this struct. 
#[derive(Debug)]
pub enum TextureDataSource {
	// storage location, dimension
	// Storage location is Some if offloaded from RAM to disk
	UserGenerated(Option<PathBuf>, wgpu::TextureViewDimension), 
	Specified(TextureDimensionAndSources),
}
impl TextureDataSource {
	pub fn dimension(&self) -> wgpu::TextureDimension {
		match self {
			&Self::UserGenerated(_, d) => match d {
				...
				_ => todo!("TextureDimension for {:?}", d),
			},
			Self::Specified(s) => s.dimension(),
		}
	}
	pub fn view_dimension(&self) -> wgpu::TextureViewDimension {
		match self {
			&Self::UserGenerated(_, d) => d,
			Self::Specified(s) => s.view_dimension(),
		}
	}
}
...
#[derive(Debug, Serialize, Deserialize)]
pub enum TextureDimensionAndSources {
...
impl TextureDimensionAndSources {
	...
	pub fn read(&self, format: TextureFormat) -> anyhow::Result<(Vec<u8>, wgpu::Extent3d)> {
		trace!("Load texture!");
		match self {
...
pub struct TextureSpecification {
	pub label: String,
	pub source: TextureDimensionAndSources,
...
impl TextureSpecification {
	pub fn from_d2_path(
		...
		Self {
			label: name.into(),
			source: TextureDimensionAndSources::D2(path.as_ref().into()),
			format,
...
pub struct Texture {
	...
	pub(super) spec_path: Option<PathBuf>, // Used only for indexing by path 
	source: TextureDataSource, // Also encodes (view)dimension
	data: Option<Vec<u8>>, // texture data, if loaded in RAM
	...
	derived_usages: RwLock<HashMap<(MaterialKey, RenderContextKey), wgpu::TextureUsages>>,
	
	dirty: AtomicBool,
	binding: Option<(wgpu::Texture, wgpu::TextureView, bool)>, // bool is for (mipped yet?)

	bind_groups: RwLock<HashSet<BindGroupKey>>,
}
impl Texture {
	pub fn from_specification_path(path: impl AsRef<Path>) -> anyhow::Result<Self> {
		...
		let (d, size) = spec.source.read(spec.format)?;
		...
		Ok(Self {
			...
			source: TextureDataSource::Specified(spec.source),
			data: Some(d),
			format: spec.format,
			size,
			...
			derived_usages: RwLock::new(HashMap::new()),
	...
	pub fn new_d2(
		...
		Self {
			label: name.into(),
			readable, writable, persistent, 

			spec_path: None,
			source: TextureDataSource::UserGenerated(None, wgpu::TextureViewDimension::D2),
			data: None,
			format,
			size: wgpu::Extent3d {
	...
	/// A convenience method for loading a texture from "whatever.png". 
	pub fn from_d2_path(
		name: impl Into<String>, 
		path: impl AsRef<Path>,
		format: TextureFormat,
		...
		let (d, s) = spec.source.read(spec.format).unwrap();

		Self {
			label: spec.label,
			readable: false, writable: true, persistent: true, 
			spec_path: Some(path.into()),
			source: TextureDataSource::Specified(spec.source),
			data: Some(d),
			format: spec.format,
	...
	pub fn usages(&self) -> wgpu::TextureUsages {
		self.derived_usages.read().values()
			.copied()
			.fold(self.base_usages, |a, u| a | u)
	}

	pub fn view(&self) -> Option<&wgpu::TextureView> {
		self.binding.as_ref().map(|(_, v, _)| v)
	}

	// Should mipping be done here? 
	// Texture manager can store a blitter for each format, then pass here for encode
	pub(crate) fn do_writes(&mut self, queue: &wgpu::Queue, _encoder: &mut wgpu::CommandEncoder) {
		if let Some((t, _, _)) = self.binding.as_ref() {
			for (i, (mip_level, origin, data)) in self.queued_writes.drain(..).enumerate() {
				debug!("Writing queued write {i} for texture '{}'", self.label);
				let bytes_per_row = std::num::NonZeroU32::new(self.format.bytes_per_element() * self.size.width).and_then(|u| Some(u.get()));
				let rows_per_image = std::num::NonZeroU32::new(self.size.height).and_then(|u| Some(u.get()));
				let size = self.size;
				queue.write_texture(
			...
			warn!("Skipping update for texture '{}' because binding does not exist", self.label);
		}
	...
	pub(crate) fn rebind(
		...
		let new_binding = {
			...
			let view = texture.create_view(&wgpu::TextureViewDescriptor {
				...
				dimension: Some(self.source.view_dimension()),
				..Default::default()
			});
		...
		let _old_binding = self.binding.replace(new_binding);
		if self.persistent {
			warn!("TODO: persist texture");
...
impl TextureManager {
	...
	pub fn insert(&mut self, texture: Texture) -> TextureKey {
		...
		let p = texture.spec_path.as_ref().and_then(|p| Some(p.clone()));
	...
	pub fn do_writes(&mut self, queue: &wgpu::Queue, encoder: &mut wgpu::CommandEncoder) {
		for texture in self.textures.values_mut() {
			texture.do_writes(queue, encoder);
		}
	}
