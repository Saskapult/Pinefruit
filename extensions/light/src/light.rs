...
// I chose to assume that torchlight will be tinted. 
// This combined with the assumption that torchlight will not cover entire chunks, allows us to have only Mixed and Empty states for this volume. 
pub struct TorchlightContents {
	// None if devoid of light 
	// Should be u24 but it (hopefully) just isn't worth it
	contents: Option<(Box<[u32]>, u32)>,
}
impl TorchlightContents {
	#[inline]
	...
	pub fn get(&self, pos: IVec3) -> u32 {
		if let Some((contents, _)) = self.contents.as_ref() {
	...
	pub fn set(&mut self, pos: IVec3, value: u32) {
		if let Some((contents, lights)) = self.contents.as_mut() {
			let i = Self::index_of(pos);
			let replaced = contents[i];
			contents[i] = value;
			if value != u32::MIN { *lights += 1 }
			if replaced != u32::MIN { *lights -= 1 }
			...
			if value != u32::MIN {
				self.contents = Some((vec![u32::MIN; CHUNK_SIZE.pow(3) as usize].into_boxed_slice(), 0));
				self.set(pos, value);
			}
		}
	}
	...
	pub fn size(&self) -> usize {
		...
		if self.contents.is_some() {
			base += 4 * CHUNK_SIZE.pow(3) as usize
		}
		base
	}
}
