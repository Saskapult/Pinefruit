...
struct Camera {
    ...
    proj: mat4x4<f32>;      // Projection matrix (ortho with specified bounds)
};
[[group(1), binding(0)]]
...
// Should switch to view space to avoid big float confusion
// Include rotation in view matrix as well
// transform our light's position using something like view_matrix * model_matrix * light_position
...
fn main(
    ...
    var projection_matrix = camera.proj;

    //var normal = model_matrix * model.normal;   // Normal at this vert's location

    var world_position: vec4<f32> = model_matrix * vec4<f32>(model.position, 1.0);
    var view_position: vec4<f32> = uniforms.view * world_position;
    var clip_positon: vec4<f32> = uniforms.proj * view_position;

    var out: VertexOutput;
    out.clip_position = clip_positon;
